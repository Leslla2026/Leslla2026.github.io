---
layout: default
title: Registration and conference fee
permalink: /registration/
---

{% include site-logo.html %}
{% include page-header.html %}

### Registration
Click on the following links to register:

- [In presence and online participation](https://leslla.wildapricot.org/event-6647304) 

- [Online only](https://leslla.wildapricot.org/event-6647372)

### Conference fee

The conference fee includes access to all sessions, coffee breaks and light refreshments, lunch on 21 August, conference badges, stationery, and a city guide. The programme and book of abstracts will be provided in digital format for environmental reasons and will be accessible via QR code. 
Conference fee **does not** include the social dinner. Participants that registered for social dinner are kindly ask to pay in cash at registration desk (price: 50 EUR or 1250 CZK).

Participants who cannot attend in person will have the possibility to participate online to the second and third day of the conference through the Zoom platform, with the exception of interactive workshops (when stated in the schedule).

Attending workshops requires **pre-registration** by **30 June**. The registration form can be found on the schedule page.

Attendance and voting at the LESLLA General assembly is **for members only**.


Wish to become a member? Please click on this [link](https://leslla.wildapricot.org/join-us) for additional details 

 <style>
  .fees-table {
    width: 50%;
    border-collapse: collapse;
  }
  .fees-table th,
  .fees-table td {
    border: 1px solid #000;
    padding: .5rem .6rem;
    vertical-align: top;
  }
  .fees-table thead th {
    background: #f5f5f5;
  }
  /* Riga di sezione (intestazione che copre entrambe le colonne) */
  .fees-section {
    background: #fafafa;
    font-weight: 700;
    color: var(--brand, #8A0044); /* usa il tuo bordeaux se definito */
  }
  
.fees-section td {
  text-align: center;
}

</style>

<table class="fees-table">
  <thead>
    <tr>
      <th>Category</th>
      <th colspan="2">Fee</th>
    </tr>
  </thead>
  <tbody>
    <!-- Member: in-person and online -->
    <tr class="fees-section">
      <td colspan="3">Member: in-person and online</td>
    </tr>
    <tr>
      <td>Early bird (by 31 May 2026)</td>     
     <td>234 USD</td>          
    </tr>
    <tr>
      <td>Regular (by 30 June 2026)</td>
     <td>328 USD</td>
    </tr>
    <tr>
      <td>After 30 June and on site</td>      
     <td>351 USD</td>
    </tr>

    <!-- Non-member: in-person & online -->
    <tr class="fees-section">
      <td colspan="3">Non-member: in-person &amp; online</td>
    </tr>
    <tr>
      <td>Early bird (by 31 May 2026)</td>
      <td>269 USD</td>
    </tr>
    <tr>
      <td>Regular (by 30 June 2026)</td>
      <td>351 USD</td>
    </tr>
    <tr>
      <td>After 30 June and on site</td>
      <td>409.50 USD</td>
    </tr>

    <!-- Student member: in-person & online -->
    <tr class="fees-section">
      <td colspan="3">Student member: in-person &amp; online</td>
    </tr>
    <tr>
      <td>Early bird (by 31 May 2026)</td>
      <td>146 USD</td>
    </tr>
    <tr>
      <td>Regular (by 30 June 2026)</td>
      <td>205 USD</td>
    </tr>
    <tr>
      <td>After 30 June and on site</td>
      <td>234 USD</td>
    </tr>

    <!-- Student non-member: in-person & online -->
    <tr class="fees-section">
      <td colspan="3">Student non-member: in-person &amp; online</td>
    </tr>
    <tr>
      <td>Early bird (by 31 May 2026)</td>
      <td>175.50 USD</td>
    </tr>
    <tr>
      <td>Regular (by 30 June 2026)</td>
      <td>234 USD</td>
    </tr>
    <tr>
      <td>After 30 June and on site</td>
      <td>263.50 USD</td>
    </tr>

    <!-- Retired/community*/LMICs: in-person & online -->
    <tr class="fees-section">
      <td colspan="3">Retired / community* / LMICs: in-person &amp; online</td>
    </tr>
    <tr>
      <td colspan="3"><em>*Without funding from their institution</em></td>
    </tr>
    <tr>
      <td>Early bird (by 31 May 2026)</td>
      <td>175.50 USD</td>
    </tr>
    <tr>
      <td>Regular (by 30 June 2026)</td>
      <td>234 USD</td>
    </tr>
    <tr>
      <td>After 30 June and on site</td>
      <td>263.50 USD</td>
    </tr>

    <!-- Member: online only -->
    <tr class="fees-section">
      <td colspan="3">Member: online only</td>
    </tr>
    <tr>
      <td>Early bird (by 31 May 2026)</td>
      <td>70 USD</td>
    </tr>
    <tr>
      <td>Regular (by 30 June 2026)</td>
      <td>140 USD</td>
    </tr>
    <tr>
      <td>After 30 June and on site</td>
      <td>211 USD</td>
    </tr>

    <!-- Non-member: online only -->
    <tr class="fees-section">
      <td colspan="3">Non-member: online only</td>
    </tr>
    <tr>
      <td>Early bird (by 31 May 2026)</td>
      <td>140 USD</td>
    </tr>
    <tr>
      <td>Regular (by 30 June 2026)</td>
      <td>211 USD</td>
    </tr>
    <tr>
      <td>After 30 June and on site</td>
      <td>234 USD</td>
    </tr>
  </tbody>
</table>
